...
import { Stagger, StaggerChild } from '../motion/motion';
...
const BookCard = (props) => {
  ...
  const [animate] = useState(() => isFirstVisit('card'))
  ...
  return (
    <motion.div
      ...
      viewport={{once: true}}
      className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4 lg:gap-6 space-y-4'>
      {
        props.books.map((book, idx) => {
          ...
          return (
            ...
            // whileInView={'show'}
            viewport={{once: true}}
            key={idx} className='group bg-white cursor-pointer p-2 shadow-2xl h-105 sm:h-100 rounded-lg relative hover:scale-102 transform duration-300'>
              <Link to={`/books/details/${book.id}`}>
                ...
                <div
                  ...
                  className={`absolute top-58 left-2 right-2 ${isAdded ? 'bg-orange-700' : ''} bg-orange-600 text-white py-2 text-[16px] font-medium opacity-0 group-hover:opacity-100 transform duration-900`}>
